...
import com.worktrace.worktracebackend.timeentry.domain.port.in.GetTimeEntriesByDateForCompanyUseCase;
...
/**
 * Implementación del caso de uso {@link GetTimeEntriesByDateForCompanyUseCase}.
 * <p>
 ...
 * degrade la consulta.
 */
public class GetTimeEntriesByDateForCompanyUseCaseImpl implements GetTimeEntriesByDateForCompanyUseCase {
    ...
    public GetTimeEntriesByDateForCompanyUseCaseImpl(TimeEntryQueryPort timeEntryQueryPort, AuthenticatedUserPort authenticatedUserPort) {
     ...
     * {@link #MAX_PAGE_SIZE}) y consulta los fichajes de la empresa del usuario
     * autenticado para la fecha indicada.
     */
    @Override
    public PageResult<AdminByDate> execute(LocalDate date, int page, int size) {
        ...
        return timeEntryQueryPort.findByCompanyIdAndWorkDate(companyId, date, safePage, safeSize);
    }
}
